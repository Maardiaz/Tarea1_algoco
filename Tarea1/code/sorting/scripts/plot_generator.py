...
TAMANOS_VALIDOS = [10, 1000, 100000]
...
def leer_mediciones(ruta_mediciones):
    ...
    for archivo in archivos:
        ...
        try:
            n = int(partes[0])
            if n not in TAMANOS_VALIDOS:
                continue
            tipo, dominio = partes[1], partes[2]
            algoritmo = partes[4].capitalize()
            ...
            print(f"Archivo no se procesa: {archivo}: {e}")
...
def generar_graficos(df, ruta_plots):
    ...
    for config in sorted(df['Configuración'].unique()):
        ...
        for metrica, info in metricas.items():
            ...
            ax.xaxis.set_major_formatter(FuncFormatter(fmt_numero))
            ax.yaxis.set_major_formatter(FuncFormatter(lambda y, _: f'{y:g}'))
            ax.set_xlim(0, ticks_x[-1] * 1.05)
            ax.set_ylim(0, df_prom[metrica].max() * 1.10)
            ax.minorticks_off()
            ax.set_title(
                f"Ordenamiento de Arreglos: {metrica.split(' ')[0]}\n"
                f"{config_display}",
                fontsize=15, fontweight='bold', pad=15
            )
            ax.set_xlabel('Cantidad de Elementos (n)', fontsize=12, fontweight='semibold')
